...
#define EVENTOS_H
	...
	typedef enum { // 1 byte size max
		...
		SET_UART			= 0x05,		// El evento ha sido pedido por la UART
		
		// Actuación del evento (Llamado por un evento de tipo SET)
		EV_GPIO_REF		= 0x06,		// El evento requiere refrescar las salidas de la IO
		EV_POWER			= 0x07,		// El evento requiere que se entre en modo PowerDown
		EV_CHECK_PULS	= 0x08,		// El evento requiere comprobar si elgún botón está pulsado
		EV_LED_ERR		= 0x09,		// El evento requiere apagar el led de error de la IO
		EV_LATIDO			= 0x0A		// El evento requiere cambiar el estado del led de latido de la IO
